...
def check_feasibility(const_coeff: List[List[float]], rhs_coeff: List[float], x: List[float]) -> bool:
...
def interior_point_method(obj_coeff: List[float], const_coeff: List[List[float]], rhs_coeff: List[float], x0: List[float]):
...
obj_coeff = list(map(int, input("Enter objective function coefficients:\n").split()))
const_coeff = [list(map(int, input("Enter constraint coefficients:\n").split())) for _ in range(m)]
rhs_coeff = list(map(int, input("Enter right hand side coefficients:\n").split()))
